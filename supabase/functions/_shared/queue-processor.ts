  ...
  try {
    ...
    if (instance.custom_escalation_enabled && instance.custom_escalation_instructions) {
      try {
        ...
        const customCheckResponse = await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/custom-escalation-checker`, {
          method: 'POST',
          headers: {
    ...
    try {
      ...
      try {
        ...
        if (intentResponse.ok) {
          ...
          logger.info('🧠 Intent analysis completed (queue)', {
            userPhone,
          ...
          if (intentAnalysis?.intent === 'external_action' && intentAnalysis?.externalAction) {
            logger.info('🚀 External action detected in queue, executing webhook:', {
              ...
              queuedMessages: messagesToProcess.length
            });

            try {
              // Get the last message data for messageId
              const lastMessageData = messagesToProcess[messagesToProcess.length - 1]?.messageData;
              ...
              const executorResponse = await fetch(`${supabaseUrl}/functions/v1/external-action-executor`, {
                ...
                body: JSON.stringify({
                  ...
                  whatsappMessageId: lastMessageData?.key?.id,
              ...
              if (executorResponse.ok) {
                ...
                logger.info('✅ External action executed successfully (queue):', {
                  ...
                  processedMessages: messagesToProcess.length
                });
                ...
                if (intentAnalysis.externalAction.responseType === 'simple_confirmation' ||
                  ...
                  if (intentAnalysis.externalAction.responseType === 'custom_message') {
                    ...
                    logger.info('🔄 Variables replaced in custom message (queue):', {
                  ...
                  logger.info('📨 Sending confirmation message to user (queue):', {
                    userPhone,
                    responseType: intentAnalysis.externalAction.responseType,
                  ...
                  try {
                    ...
                    const lastMessageData = messagesToProcess[messagesToProcess.length - 1]?.messageData;
                    const evolutionApiKey = Deno.env.get('EVOLUTION_API_KEY') || lastMessageData?.apikey;
                    ...
                    if (!evolutionApiKey) {
                      ...
                      if (sendResponse.ok) {
                        logger.info('✅ Confirmation message sent successfully via WhatsApp (queue)');
                      } else {
                        logger.error('❌ Failed to send confirmation message via WhatsApp (queue):', {
                          status: sendResponse.status,
                          statusText: sendResponse.statusText
                        });
                      }
                    ...
                    logger.error('❌ Error sending confirmation message (queue):', {
                  ...
                  try {
                    ...
                    if (insertError) {
                      logger.error('❌ Failed to create pending response record (queue):', {
                        error: insertError.message,
                        executionLogId: executorResult.executionLogId
                      });
                    } else {
                      logger.info('⏳ Pending response record created successfully (queue):', {
                        executionLogId: executorResult.executionLogId,
                        userPhone,
                        instanceName,
                        timeoutSeconds,
                    ...
                    logger.error('❌ Exception creating pending response record (queue):', {
                  ...
                  logger.info('ℹ️ Response type does not require immediate message send (queue):', {
                    responseType: intentAnalysis.externalAction.responseType
                  });
                }

                // Mark messages as completed before returning
                await markMessagesAsCompleted(messagesToProcess);
                ...
                logger.error('❌ External action executor returned error (queue):', {
              ...
              logger.error('❌ Exception calling external-action-executor (queue):', {
