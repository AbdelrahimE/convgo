...
export class CustomerProfileManager {
  ...
  /**
   * Extract customer info using gpt-4o-mini AI with conversation context
   */
...
Extract and return ONLY a JSON object with these fields (set to null if not found or unchanged):
{
  ...
  "journey_stage": "first_time|researching|ready_to_buy|existing_customer"
      ...
      // AI insights
      if (extractedInfo.customer_stage && extractedInfo.customer_stage !== null) {
        updates.customer_stage = extractedInfo.customer_stage;
      }
  ...
  /**
   * Get enhanced context for AI including customer profile and conversation summaries
   */
  async getEnhancedContext(instanceId: string, phoneNumber: string): Promise<string> {
    try {
      ...
      // Add customer stage and AI insights
      context += `Customer Stage: ${profile.customer_stage}\n`;
      
      if (profile.customer_intent) {
        context += `Customer Intent: ${profile.customer_intent}\n`;
      }
      
      if (profile.customer_mood) {
        context += `Customer Mood: ${profile.customer_mood}\n`;
      }
      ...
      context += `Total Messages: ${profile.total_messages}, AI Interactions: ${profile.ai_interactions}\n`;
