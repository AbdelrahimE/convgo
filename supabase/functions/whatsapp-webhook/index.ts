...
serve(async (req) => {
  ...
  try {
    ...
    const isConnectionUpdatePath = url.pathname.includes('/connection-update');
    if (isConnectionUpdatePath && data && data.instance) {
      await logDebug('CONNECTION_UPDATE_PATH', `Detected connection update webhook for instance: ${data.instance}`);
      
      await saveWebhookMessage(data.instance, 'connection.update', data);
      await processConnectionStatus(data.instance, data);
      
      return new Response(JSON.stringify({ 
        success: true,
        message: `Connection state processed for instance ${data.instance}`
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
