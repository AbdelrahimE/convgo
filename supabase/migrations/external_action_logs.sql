create table public.external_action_logs (
  ...
  constraint external_action_logs_execution_status_check check (((execution_status)::text = any (array[('pending'::character varying)::text, ('success'::character varying)::text, ('failed'::character varying)::text, ('timeout'::character varying)::text]))),
  constraint external_action_logs_intent_confidence_check check (((intent_confidence >= (0.0)::numeric) and (intent_confidence <= (1.0)::numeric))),
  constraint external_action_logs_retry_count_check check (((retry_count >= 0) and (retry_count <= 10)))
